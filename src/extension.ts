...
export async function activate(context: vscode.ExtensionContext) {
  ...
  const configuredPython = cfg.get<string>("pythonPath") || "python";
  const pythonPath = resolvePythonPath(configuredPython);
  if (!pythonPath) {
    vscode.window.showErrorMessage(
      "XSS LSP: Python executable not found. Set xssLsp.pythonPath to python3 or a full path."
    );
    return;
  }
...
function resolvePythonPath(configured: string): string | undefined {
...
async function reportXssDiagnostics(uris: readonly vscode.Uri[]) {
  if (!outputChannel) {
    return;
  }
  ...
  for (const uri of uris) {
    ...
    for (const diag of diagnostics) {
      ...
      outputChannel.appendLine(
        `[XSS] ${path.basename(uri.fsPath)}:${line + 1}:${col + 1} ${diag.message}`
      );
